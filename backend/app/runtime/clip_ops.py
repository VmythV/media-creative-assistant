...
def apply_clip_ops(base_plan_id: int, ops: list[dict]) -> dict:
    ...
    ir = carry_ir_settings(plan_to_ir(new_plan, analyzed, base_plan.get("title") or "局部修订"), base_ir)
...
def add_title_card(base_plan_id: int, *, text: str, subtitle: str = "",
    ...
    analyzed = _load_analyzed_assets(None)
    new_plan = {**base_plan, "clips": clips}
    ir = carry_ir_settings(plan_to_ir(new_plan, analyzed, base_plan.get("title") or "加标题卡"), base_ir)
    validate_ir(ir)
