...
## 5. Editing IR Schema（v0.1 → v0.2）

> v0.2（M8）：启用 `audio` 轨，items 为单条 `music`（source_id/gain_db/fade_in/fade_out/loop），配乐由 API 确定性写入而非模型产出；校验器兼容 0.1 存量。详见 §11。

核心结构（Pydantic 定义为准，此处为 v0.1 示例）：
...
- **IR v0.2**：启用音频轨。`AudioTrack(type="audio")` 的 items 为 `MusicClip(type="music", source_id, gain_db=-16, fade_in=1s, fade_out=2s, loop=true)`；MVP 限制每条音频轨仅 1 条配乐（多段配乐/精确对位放后续）。校验器接受 version 0.1 与 0.2（0.1 存量方案兼容），新生成 IR 标 0.2。配乐**不经过模型**：由用户通过 API 指定文件，确定性写入 IR（延续"模型只产出受限方案格式"的风险控制）。
- **API**：`PUT /api/plans/{plan_id}/music` body `{path, gain_db?, fade_in?, fade_out?, loop?}` 设置/替换配乐（校验文件存在且含音频流）；`DELETE` 移除。直接改 plan.ir 并重新校验落库，不改方案状态。
- **渲染器**：拼接后新增混音 pass（视频流 copy）：配乐 loop/截齐时间线长度 → 音量 → 首尾 afade → 与主音频 `amix`（normalize=0，保留对白响度）。
- **Resolve**：配乐文件随素材导入媒体池（脚本 API 无法可靠地把音频放到时间线指定位置，与 SRT 同样一步拖入；限制记录于此）。
  > 勘误（2026-07-13）：实测 `AppendToTimeline` clipInfo 的 `mediaType=2 + trackIndex + recordFrame` 可以精确放置音频，当时误判源于视频片段自带联动音轨。见 `docs/resolve-scripting-api.md` §5。
- **成片预览**：FastAPI 挂载 `/output` → `data/output`（注册在前端根挂载之前）；渲染结果附 `video_url`，前端成片卡片内 `<video controls>` 直接播放。

| 里程碑 | 内容 | 验证方式 |
| --- | --- | --- |
| M8 配乐与预览 | IR v0.2 音频轨 + 配乐 API + 渲染混音 + 成片托管/预览 | pytest：schema 校验、配乐 API、混音后成片含音频；真实方案挂配乐渲染并在浏览器播放 |

---

## 12. M9：转场效果

- **IR v0.3**：`Clip` 增加可选 `transition` 字段，语义为**与前一片段之间的转场**（转场进入本片段）：`{"type": <白名单>, "duration": 0.5}`。类型白名单取自 ffmpeg `xfade` 滤镜（本机精简编译已确认可用）：`fade / fadeblack / fadewhite / dissolve / wipeleft / wiperight / slideleft / slideright / circleopen / circleclose`。约束：时长 (0, 2] 秒；首个片段 transition 必须为 null；每个片段需满足「转入时长 + 转出时长 < 片段时长」（xfade 重叠消耗两侧素材）。校验器接受 0.1/0.2/0.3，新生成 IR 标 0.3。
- **时间轴语义**：转场消耗重叠——总时长 = Σ片段时长 − Σ转场时长（`timeline_duration` 相应扣减，配乐截齐自动跟随）。确定性转换中，字幕窗口取片段的"独占时间槽" `[pos, pos + 片段长 − 转入时长]`，pos 按独占长度累加：转场期间显示上一条字幕、新字幕从转场结束起显示，天然满足字幕不重叠校验。
- **方案格式**：clip 增加 `"transition": {"type": ..., "duration": ...} 或 null`，由 Planning 模型按节奏选择（叙事升温用 dissolve/fade、场景切换用 wipe/slide、收尾用 fadeblack 等，提示词给出指引）；确定性转换负责钳制：类型不在白名单则忽略、时长 clamp 到 `min(2.0, 前段长/2, 本段长/2)`——模型给出越界值时自动修复而非报错，延续"受限格式 + 确定性转换"的风险控制。修订指令可自然增删转场，`diff_plans` 增加转场变化检测。
- **渲染器**：无任何转场时保持原 concat 流复制快路径；存在转场时单次 ffmpeg filter_complex 链式处理全部边界（一次重编码，无代际损失）：有转场的边界用 `xfade=transition=T:duration=d:offset=(累计输出长−d)` + 音频 `acrossfade=d`，硬切边界用 `concat` 滤镜（视频/音频各自成链），逐段折叠到最终输出。各片段已在第一步统一规格（分辨率/fps/像素格式/48k 立体声），满足 xfade 同规格要求。offset 用名义 trim 时长累计（与字幕时间轴一致、确定性）。
- **Resolve**：脚本 API 无法在时间线片段间插入转场（无公开接口），Resolve 时间线仍为硬切，执行摘要中记录 `transitions.method = "unsupported"` 提示在 Resolve 内手动添加；转场完整效果体现在渲染成片。
  > 补充（2026-07-13）：实测发现可行的间接路径——生成含 `<transition>` 的 FCPXML 后经 `ImportTimelineFromFile` 建时间线，转场对象真实导入且往返导出无损。列为后续里程碑候选，见 `docs/resolve-scripting-api.md` §5/§8。
- **前端**：方案片段列表在片段前显示转场标记（类型中文名 + 时长）。

| 里程碑 | 内容 | 验证方式 |
| --- | --- | --- |
| M9 转场 | IR v0.3 transition + 转换钳制/字幕时移 + xfade/acrossfade 渲染链 + 提示词与前端展示 | pytest：schema 约束、转换钳制与字幕时移、diff 转场检测、成片时长=Σ片段−Σ转场（含混合硬切）；真实方案带转场渲染走查 |

---

## 13. M10：Resolve 时间线带转场与配乐（基于 §12 勘误的两项实测能力）

- **FCPXML 转场路径**：IR 含转场时，Resolve 执行不再走 AppendToTimeline 逐片段，改为生成含 `<transition>` 的 FCPXML 1.9 → `MediaPool.ImportTimelineFromFile`（素材随导入自动入媒体池，按 `media-rep src` 路径匹配，实测可靠）。无转场时保持原 AppendToTimeline 路径不动（成熟且少一次落盘）。
- **时间轴映射（居中对齐）**：IR 转场语义与 ffmpeg xfade 一致（总长 = Σ片段 − Σ转场，混合区为前段末 t 秒 × 后段首 t 秒）。FCPXML 用居中转场精确复现：转入时长 t 的片段媒体入点 +t/2、spine 时长 −t/2；转出侧 spine 时长再 −t/2；`<transition offset=剪辑点−t/2, duration=t>`。两侧 handle 恰好消耗 IR trim 范围内的素材（校验器已保证 trim 在素材内且 t_in+t_out < 片段长），成片、字幕时间轴、Resolve 时间线三者帧级一致。
- **转场类型映射**（2026-07-13 逐名实测，词汇表见 `docs/resolve-scripting-api.md` §5.3）：Resolve 按 FCPX 效果名识别，10 种 IR 类型映射 4 种 Resolve 转场——fade/dissolve→Cross Dissolve（交叉叠化）、fadeblack/fadewhite→Fade To Color（浸入颜色叠化）、wipeleft/wiperight→Wipe、slideleft/slideright→Slide（均为边缘划像）、circleopen/circleclose→Circle（椭圆展开）；方向/颜色参数不被导入器识别（往返验证被重置为默认），需在 Resolve 内微调；未知名安全回退交叉叠化。渲染成片仍保留全部精确类型。
- **配乐入轨**：`AppendToTimeline([{mediaPoolItem, startFrame, endFrame, mediaType: 2, trackIndex: 1, recordFrame: timeline.GetStartFrame()}])` 直接放 A1 轨（音频轨不足时先 AddTrack），配乐按时间线长度截齐（endFrame 换算媒体帧）。gain/fade 为脚本 API 空白（无片段音量接口），响度处理仍属渲染侧；摘要 `music.method = "timeline"`，失败降级 "media_pool" 不阻断。
- **前端**：执行结果卡展示转场（fcpxml_import：已带转场/统一叠化）与配乐（timeline：已上 A1 轨）状态。

| 里程碑 | 内容 | 验证方式 |
| --- | --- | --- |
| M10 Resolve 转场配乐 | FCPXML 转场导出（居中对齐数学）+ ImportTimelineFromFile 执行路径 + 配乐 recordFrame 入轨 | pytest：FCPXML 转场元素/offset 数学/总长=IR 时长、无转场输出不变；真实方案（含转场+配乐）执行后 Resolve 时间线含转场对象与 A1 配乐 |

---

## 14. M11：User Memory（用户创作偏好记忆）

需求文档 5.10 要求 Memory 接口化并支持多种记忆类型；MVP 边界原为"User Memory 仅留接口"，现正式实现（Global/Business 仍留枚举）。

- **MemoryProvider 接口**（需求 322：存储必须接口化）：`list / add / delete / texts`，SQLite 实现（`memory_items` 表：kind/content/source/created_at）；kind 枚举 user/project/temporary/global/business，本里程碑实现 **user**。`get_memory_provider()/set_memory_provider()` 与 LLM/Vision Provider 同模式，可注入可替换。
- **偏好提取（风险控制同方案生成）**：修订方案**成功后**，LLM 从修订指令中提取"对未来所有项目都适用的持久偏好"（受限格式 `{"preferences": ["≤30字/条", ...]}`，最多 3 条；一次性指令如"这条压到20秒"必须返回空）。确定性写入：内容归一化去重、七类记忆词汇表外的 kind 拒绝。提取失败静默跳过，不影响修订主流程。
- **注入**：`generate_plan` / `revise_plan` 组装消息时加载 user 记忆，附加到 system prompt：「用户长期创作偏好（历史沉淀，若与本次目标冲突以目标为准）」。空记忆零开销。
- **API**：`GET /api/memory`（列表）、`POST /api/memory`（手动添加，body `{content}`）、`DELETE /api/memory/{id}`。
- **前端**：新「偏好记忆」页签：偏好列表（含来源/时间、删除）+ 手动添加输入框 + 说明文案（自动学习来自修订指令）。
- **不做**（后续）：向量检索、Project/Temporary 记忆迁移（现有素材缓存与会话机制已覆盖其职责）、偏好置信度/衰减。

| 里程碑 | 内容 | 验证方式 |
| --- | --- | --- |
| M11 User Memory | MemoryProvider + SQLite + 修订偏好提取 + 生成注入 + 管理 API/前端 | pytest：provider 去重/CRUD、提取受限格式（一次性指令为空）、注入进 system prompt、修订触发提取；真实修订指令沉淀偏好并影响下一次生成 |
