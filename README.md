...
使用流程：素材页输入视频/照片目录路径导入（照片自动转成 4 秒推近片段）→ 分析全部 → 查看分类/精彩片段推荐 → 方案页输入创作目标（如"60秒旅行短片，中文字幕"）→ 确认方案 → 执行（在 Resolve 中生成时间线；SRT 字幕已导入媒体池，右键 → Insert Selected Subtitles to Timeline 上轨）→ 可对方案自然语言修订（带差异对比与回滚）、设置背景音乐 → "渲染成片"输出带字幕/配乐的 mp4，并可在浏览器内直接预览播放。
