...
class ClipOp(BaseModel):
    op: Literal["trim", "remove", "move", "subtitle", "transition", "replace", "speed"]
    position: int  # 1 起，基于执行到该步时的列表
    duration: float | None = None   # trim：目标时长（从原起点保留）
    start: float | None = None      # trim：素材内新起点
    end: float | None = None        # trim：素材内新终点
    to: int | None = None           # move：目标位置
    text: str | None = None         # subtitle：新字幕（空串清除）
    type: str | None = None         # transition：类型或 none
    t_duration: float | None = None  # transition：时长
    asset_id: int | None = None     # replace：限定素材
    hint: str | None = None         # replace：语义提示词（匹配片段描述）
    speed: float | None = None      # speed：0.25-4.0（>1 快放、<1 慢动作、1 复原）


class EditClipsParams(BaseModel):
    ops: list[ClipOp]
    plan_id: int | None = None


class AddTitleParams(BaseModel):
    text: str
    subtitle: str = ""
    position: Literal["intro", "outro"] = "intro"
    duration: float = 2.5
    plan_id: int | None = None


class PublishKitParams(BaseModel):
    platform: str = "抖音"
    plan_id: int | None = None
...
INTENT_PARAMS: dict[str, type[BaseModel]] = {
    ...
    "publish_kit": PublishKitParams,
    "edit_clips": EditClipsParams,
    "review_video": PlanRefParams,
    "fix_issues": PlanRefParams,
    "add_title": AddTitleParams,
    "smart_crop": PlanRefParams,
}


async def _act_edit_clips(session: dict, params: dict) -> dict:
    from app.runtime.clip_ops import apply_clip_ops

    plan_id = _resolve_plan_id(session, params)
    result = apply_clip_ops(plan_id, params["ops"])
    session["context"]["plan_id"] = result["plan_id"]
    return {"plan_id": result["plan_id"], "revised_from": plan_id,
            "changes": "；".join(result["changes"]), "duration": result["duration"]}


async def _act_smart_crop(session: dict, params: dict) -> dict:
    from app.runtime.framing import smart_crop

    plan_id = _resolve_plan_id(session, params)
    result = await smart_crop(plan_id)
    session["context"]["plan_id"] = result["plan_id"]
    return {"plan_id": result["plan_id"], "revised_from": plan_id, "change": result["change"]}


async def _act_add_title(session: dict, params: dict) -> dict:
    from app.runtime.clip_ops import add_title_card

    plan_id = _resolve_plan_id(session, params)
    result = add_title_card(plan_id, text=params["text"], subtitle=params.get("subtitle", ""),
                            position=params.get("position", "intro"),
                            duration=params.get("duration", 2.5))
    session["context"]["plan_id"] = result["plan_id"]
    return {"plan_id": result["plan_id"], "revised_from": plan_id, "change": result["change"]}


async def _act_review_video(session: dict, params: dict) -> dict:
    from app.runtime.review import review_render

    plan_id = _resolve_plan_id(session, params)
    review = await review_render(plan_id)
    issues_text = "；".join(
        f"[{i['severity']}] {i['detail']}" + (f"（建议：{i['suggestion']}）" if i.get("suggestion") else "")
        for i in review["issues"]
    ) or None
    return {"plan_id": plan_id, "verdict": review["verdict"],
            "summary": review["summary"], "issues": issues_text}


async def _act_fix_issues(session: dict, params: dict) -> dict:
    from app.runtime.review import apply_review_fixes

    plan_id = _resolve_plan_id(session, params)
    result = await apply_review_fixes(plan_id)
    if result["fixed"]:
        session["context"]["plan_id"] = result["new_plan_id"]  # 后续操作落到修好的新方案
        return {"plan_id": result["new_plan_id"], "revised_from": plan_id,
                "applied": "；".join(result["applied"]),
                "manual": "；".join(result["manual"]) or None,
                "note": "已生成修复后的新方案，可重新渲染查看"}
    return {"plan_id": plan_id, "fixed": False, "message": result["message"]}


async def _act_publish_kit(session: dict, params: dict) -> dict:
    from app.runtime.publish import generate_publish_kit

    plan_id = _resolve_plan_id(session, params)
    with db_session() as db:
        plan = db.get(EditPlan, plan_id)
        if plan is None or not plan.plan.get("clips"):
            raise ValueError(f"方案 #{plan_id} 不存在或没有内容")
        plan_dict = dict(plan.plan)
    kit = await generate_publish_kit(plan_dict, params.get("platform", "抖音"))
    with db_session() as db:
        plan = db.get(EditPlan, plan_id)
        plan.plan = {**plan.plan, "publish": kit}
        db.commit()
    return {"plan_id": plan_id, **kit}
...
- revise_plan {"instruction": 修订指令, "plan_id"?: 方案id}：AI 重排整个方案——用于模糊/大结构调整（"节奏快一点""重新编排下叙事"）
- edit_clips {"ops": [操作...], "plan_id"?}：**精确局部修改**（不重新生成，秒级完成）——用户指明了改哪个片段怎么改时优先用这个。ops 按序执行，position 从 1 起：
  · {"op":"trim","position":N,"duration":秒} 或 {"op":"trim","position":N,"start":s,"end":e}：改时长/区间
  · {"op":"remove","position":N}：删除片段
  · {"op":"move","position":N,"to":M}：调整顺序
  · {"op":"subtitle","position":N,"text":"新字幕"}：改字幕（空串清除）
  · {"op":"transition","position":N,"type":"fade|dissolve|fadeblack|wipeleft|...|none","t_duration"?:秒}：改转场
  · {"op":"replace","position":N,"hint"?:"关键词如 夕阳"}：换素材（从未用过的精彩片段中选）
  · {"op":"speed","position":N,"speed":0.25-4.0}：变速（>1 快放/延时，<1 慢动作，1 复原）。"第3段放慢一倍"→speed 0.5，"开头快放两倍"→speed 2
  例："开头缩短到2秒，第三段删掉" → [{"op":"trim","position":1,"duration":2},{"op":"remove","position":3}]
...
- publish_kit {"platform"?: 平台名默认抖音, "plan_id"?}：生成发布文案包（标题/简介/话题标签），"帮我写个文案/标题"用这个
- review_video {"plan_id"?}：成片质量自检（时长偏差/黑场/响度/重复素材 + AI 看片检查画质字幕观感），"检查一下成片/有没有问题"用这个
- fix_issues {"plan_id"?}：按自检报告一键自动修复（时长偏长→修剪、黑场/重复素材→替换），产出修好的新方案。"按建议改/帮我修一下/检查并修复"用这个（无自检报告会先自检）
- add_title {"text": 标题文字, "subtitle"?: 副标题, "position"?: "intro|outro", "duration"?: 秒}：加片头/片尾标题卡（纯色背景居中文字）。"加个片头叫XX/片尾加个落款"用这个，产出新方案
- smart_crop {"plan_id"?}：竖屏智能裁切——视觉定位每段主体，裁切窗口跟随主体填满竖屏（避免上下模糊边、主体不居中被切）。"竖屏别切到主体/智能裁切填满"用这个（需已设竖屏/方形输出）
...
6. revise_plan / edit_clips / fix_issues 会产生**新方案**。同一批 actions 中，紧随其后的 render/execute/publish_kit/review_video 等**不要填 plan_id**（留空），系统会自动作用于刚产生的新方案；填了旧 plan_id 会操作到未修改的旧方案。
...
- 画中画/多视频轨 → 后续版本支持，当前请在 Resolve 中手动叠轨

当前系统状态：
{state}"""


def _state_brief(session: dict | None = None) -> str:
    """给调度员的系统状态简报（紧凑，供指代解析与咨询回答）。

    含当前方案的片段清单：edit_clips 的 position（"最后一段""第3段"）必须以此锚定。
    """
    lines = []
    with db_session() as db:
        ...
        # 当前方案片段清单（会话上下文优先，缺省最新方案）
        current_id = (session or {}).get("context", {}).get("plan_id") or (plans[0].id if plans else None)
        current = db.get(EditPlan, current_id) if current_id else None
        if current and current.plan.get("clips"):
            clips = current.plan["clips"]
            lines.append(f"当前方案 #{current.id} 共 {len(clips)} 个片段：")
            for i, c in enumerate(clips, 1):
                if c.get("kind") == "title":
                    lines.append(f"  {i}. 标题卡「{c.get('text', '')}」{c.get('duration', 2.5):.1f}s")
                    continue
                t = c.get("transition")
                tl = (c["end"] - c["start"]) / float(c.get("speed") or 1.0)
                sub = f" 字幕«{c['subtitle']}»" if c.get("subtitle") else ""
                trans = f" 转场{t['type']}" if t else ""
                spd = f" {c['speed']}x" if c.get("speed", 1) != 1 else ""
                lines.append(f"  {i}. {tl:.1f}s{spd}{sub}{trans}")
...
async def route_message(session: dict, message: str) -> dict:
    ...
    messages = [
        ...
        {"role": "system", "content": CHAT_SYSTEM_PROMPT.replace("{state}", _state_brief(session))},
        *history,
...
ACTION_IMPL = {
    ...
    "publish_kit": _act_publish_kit,
    "edit_clips": _act_edit_clips,
    "review_video": _act_review_video,
    "fix_issues": _act_fix_issues,
    "add_title": _act_add_title,
    "smart_crop": _act_smart_crop,
}


PLAN_PRODUCING = {"revise_plan", "edit_clips", "fix_issues", "add_title", "smart_crop"}


async def run_actions(session_id: str, actions: list[dict]) -> None:
    """串行执行动作；失败中断后续（标 skipped）。结果追加进会话消息流。

    链内 plan 重映射：revise/edit_clips/fix_issues 产生新方案后，后续动作里
    指向旧方案 id 的 plan_id 自动改写为新 id（模型按执行前状态排 action，
    无法预知新 id；防止渲染/执行落到未修改的旧方案）。
    """
    session = _load_session(session_id)
    failed = False
    remap: dict[int, int] = {}
    for action in actions:
        # 后续动作里被超越的旧 plan_id 改写为新方案
        pid = action.get("params", {}).get("plan_id")
        if pid in remap:
            action["params"]["plan_id"] = remap[pid]
        ...
        try:
            ...
            if action["intent"] in PLAN_PRODUCING and result.get("revised_from") and result.get("plan_id"):
                remap[result["revised_from"]] = result["plan_id"]
